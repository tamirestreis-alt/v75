...
- **Conteúdo Extraído:** {search_results.get('statistics', {}).get('content_extracted', 0):,} caracteres
...
{chr(10).join(f"- {provider}" for provider in search_results.get('providers_used', []))}
...
"""
    ...
    if youtube_results:
        ...
        for i, result in enumerate(youtube_results[:10], 1):
            ...
            report += f"**Views:** {result.get('view_count', 0):,}  \n"
            report += f"**Likes:** {result.get('like_count', 0):,}  \n"
            report += f"**Comentários:** {result.get('comment_count', 0):,}  \n"
    ...
    if social_results:
        ...
        for i, result in enumerate(social_results[:10], 1):
            ...
            report += f"**URL:** {result.get('url', 'N/A')}  \n"
            report += f"**Conteúdo:** {result.get('content', 'N/A')[:150]}...  \n\n"
    ...
    if screenshots:
        ...
        for i, screenshot in enumerate(screenshots, 1):
            ...
            report += f"![Screenshot {i}]({screenshot.get('relative_path', '')})  \n\n"
